...
void printStats(unsigned int *initialProbes, unsigned int htCapacity)
{
  ...
  cout << endl
       << "There are " << results[0] << " empty cells in this Dictionary (hash table) -> \t"
  ...
  {
    if (results[k] > 0)
      cout << "There were " << results[k] << " indexing keys that hashed into cells " << k << " times -> \t" << setprecision(4) << results[k] / ((float)htCapacity) * 100
